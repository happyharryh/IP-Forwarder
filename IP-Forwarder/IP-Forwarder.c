...
TcpListenerThreadProc(LPVOID Context)
{
    ...
    for (;;) {
        ...
        targetAddress.sin_port = tuple->port;
...
UdpRelayThreadProc(LPVOID Context)
{
    ...
    localTarget.sin_port = tuple->port;
...
void NETIOAPI_API_ IpInterfaceChangeNotificationCallback(PVOID context, PMIB_IPINTERFACE_ROW row, MIB_NOTIFICATION_TYPE type)
{
...
void ResetLogFile(BOOL standaloneExe)
{
    ...
    // Print a log header
    printf("IP Forwarder v" VER_VERSION_STR "\n");
...
int Run(BOOL standaloneExe)
{
    ...
    HANDLE ifaceChangeEvent = CreateEvent(NULL, TRUE, FALSE, NULL);
    ...
    if (err != 0) {
        printf("Failed to open rules.conf\n");
        return err;
    }
